...
### [2026-09-12T06:46:00+05:30] Deployment 18: OMS Provider Health Recovery & Timezone-Aware Daily Equity Refresh
* **Issue**: (1) Inactive provider recovery needed to happen outside the hot path in OMS via scheduled gRPC probes while enforcing incomplete configuration rules (no default strings). (2) Multi-broker day loss calculations required region/exchange-aware starting equity resets based on each broker's local timezone.
* **Root Cause Analysis (RCA)**: Performing health checks or Redis status queries in OPS hot path introduces unnecessary network latency (< 1µs in-memory check preferred). Automated day loss checks require resetting `balance:starting_equity:<provider>` at midnight in the broker's local timezone (e.g. `America/New_York` vs `Asia/Kolkata`).
* **Fix Applied**:
  1. Updated [`ProviderConfig.java`](file:///c:/Users/jeshu/Projects/distributed-trading-system/CombinedOrderingSystem/libs/shared-models/src/main/java/com/trading/shared/config/ProviderConfig.java): Added `timezone`, `exchange`, transient `active` flag, and `isConfigComplete()` validator (strictly enforcing non-blank fields without defaults).
  2. Updated [`RiskManager.java`](file:///c:/Users/jeshu/Projects/distributed-trading-system/CombinedOrderingSystem/ms/order-processing-service/src/main/java/com/trading/ops/service/RiskManager.java): Pre-trade firewall checks `config.isConfigComplete()` and `config.isActive()` in memory (< 1µs RTT avoided). Added `markProviderActive/Inactive` methods.
  3. Created [`ProviderHealthCheckJob.java`](file:///c:/Users/jeshu/Projects/distributed-trading-system/CombinedOrderingSystem/ms/order-management-service/src/main/java/com/trading/oms/job/ProviderHealthCheckJob.java) in OMS: Periodically probes gRPC health for complete inactive providers every 15s and restores status to `ACTIVE` in Redis upon recovery. Skips incomplete configurations.
  4. Created [`DailyEquityRefreshJob.java`](file:///c:/Users/jeshu/Projects/distributed-trading-system/CombinedOrderingSystem/ms/order-management-service/src/main/java/com/trading/oms/job/DailyEquityRefreshJob.java) in OMS: Evaluates date rollover based on `ZoneId.of(provider.getTimezone())` and resets `balance:starting_equity:<provider>` at midnight local time.
  5. Committed all changes (`commit b1d9ffe`: `feat(oms): implement provider health check retry scheduler and timezone-aware daily equity refresh`) and pushed to `origin/master`.
### [2026-09-15T22:26:00+05:30] Deployment 19: Direct Broker gRPC Daily Equity Refresh & Decoupled OMS Equity Reconciliation Service
* **Issue**: Daily equity refresh previously calculated `starting_equity` from internal Redis keys (`balance:cash`, `positions:*`, and cached prices). This created a closed-loop state drift where missed intraday execution messages or stale prices permanently corrupted the daily drawdown baseline in `balance:starting_equity:<provider>`. Additionally, equity reconciliation logic was coupled inside `@Scheduled` annotations.
* **Root Cause Analysis (RCA)**: Closed-loop internal calculation preserves and compounds intraday state drift over time. Relying on cached ticker prices at midnight rollover can result in fallback valuation errors. Direct broker gRPC query provides authoritative clearinghouse ground-truth with zero rate-limit impact (runs 1x/day). Decoupling reconciliation logic into a service layer enables on-demand mid-day triggers (e.g., via UI or admin endpoints).
* **Fix Applied**:
  1. Updated [`connection_manager.proto`](file:///c:/Users/jeshu/Projects/distributed-trading-system/CombinedOrderingSystem/libs/shared-models/src/main/proto/connection_manager.proto): Added `AccountService` interface with `GetAccountDetails` RPC method and `AccountDetailsResponse` message.
  2. Updated `connection-manager-alpaca`: Added `get_account` to `AlpacaRestClient` ([`alpaca_client.py`](file:///c:/Users/jeshu/Projects/distributed-trading-system/connection-manager-alpaca/alpaca_client.py)) and implemented `AccountServiceServicer` in [`grpc_server.py`](file:///c:/Users/jeshu/Projects/distributed-trading-system/connection-manager-alpaca/grpc_server.py).
  3. Updated [`ReconciliationClient.java`](file:///c:/Users/jeshu/Projects/distributed-trading-system/CombinedOrderingSystem/ms/order-management-service/src/main/java/com/trading/oms/service/ReconciliationClient.java): Added `getAccountDetails(String provider)` gRPC client method.
  4. Created [`EquityReconciliationService.java`](file:///c:/Users/jeshu/Projects/distributed-trading-system/CombinedOrderingSystem/ms/order-management-service/src/main/java/com/trading/oms/service/EquityReconciliationService.java): Decoupled service handling `reconcileDailyStartingEquity` (broker gRPC query + Redis cash resync + internal fallback) and `reconcileLiveBalance` (on-demand / UI mid-day triggers).
...
* **Fix Applied**:
  ...
  2. Updated [`CombinedOrderingSystem/libs/shared-models/pom.xml`](file:///c:/Users/jeshu/Projects/distributed-trading-system/CombinedOrderingSystem/libs/shared-models/pom.xml) to use `maven-resources-plugin` to copy `../../../config/grpc-proto-schema/*.proto` (3-level relative depth from `shared-models`) into `${project.build.directory}/proto-schema` and configured `protobuf-maven-plugin` `<protoSourceRoot>${project.build.directory}/proto-schema</protoSourceRoot>`.
