...
7. **Git Read Access**: The agent is granted pre-approved access to run all git read-only commands (e.g. `git status`, `git log`, `git diff`, `git show`, `git branch`, `git rev-parse`, `git grep`, `git ls-files`, `git blame`, `git check-ignore`, `git remote`, `git tag`, `git describe`, `git cat-file`, `git reflog`, `git shortlog`, `git merge-base`, `git config --get`, `git config --list`) directly without requiring prior user permission or approval.
